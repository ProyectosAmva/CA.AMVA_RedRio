...
/// <summary>
    ...
    /// </summary>
namespace AMVA.REDRIO.Infrastructure.Repositories;
...
public class HistorialExcelRepository : IRepositoryHistorialExcel<HistorialExcel>
    {
        ...
        {
            return await _context.HistorialExcels
             .Include(r => r.Campaña)  
                .ToListAsync();
        }
        ...
        public async Task<IEnumerable<HistorialExcel>> GetByCampañasAsync(int IdCampaña)
    {
        ...
        {
            _context.HistorialExcels.Add(historialExcel);
            await _context.SaveChangesAsync();
        }

        public async Task UpdateAsync(HistorialExcel historialExcel)
        {
            _context.HistorialExcels.Update(historialExcel);
            await _context.SaveChangesAsync();
        }

        public async Task DeleteAsync(int id)
        {
            var historialExcel = await _context.HistorialExcels.FindAsync(id);
            if (historialExcel != null)
            {
                _context.HistorialExcels.Remove(historialExcel);
                await _context.SaveChangesAsync();
            }
        }
    }
